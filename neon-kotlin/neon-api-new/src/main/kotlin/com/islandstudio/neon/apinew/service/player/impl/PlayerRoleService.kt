package com.islandstudio.neon.apinew.service.player.impl

import com.islandstudio.neon.apinew.connection.UserContext
import com.islandstudio.neon.apinew.dto.ResultProvider
import com.islandstudio.neon.apinew.dto.action.player.role.CreatePlayerRoleActionDTO
import com.islandstudio.neon.apinew.dto.action.player.role.UpdatePlayerRoleActionDTO
import com.islandstudio.neon.apinew.entity.player.PlayerRole
import com.islandstudio.neon.apinew.repository.player.IPlayerRoleRepository
import com.islandstudio.neon.apinew.service.player.IPlayerRoleService
import com.islandstudio.neon.apinew.status.ActionResultStatus
import com.islandstudio.neon.shared.core.di.IComponentProvider
import com.islandstudio.neon.shared.core.di.getComponent
import org.koin.core.annotation.Single

@Single
class PlayerRoleService: IPlayerRoleService, IComponentProvider {
    private val playerRoleRepository = getComponent<IPlayerRoleRepository>()

    override suspend fun createPlayerRole(context: UserContext, action: CreatePlayerRoleActionDTO): ResultProvider<PlayerRole> {
        /* Check role existence by code */
        playerRoleRepository.isExistByCode(action.code).also {
            if (!it) return@also

            return ResultProvider(status = ActionResultStatus.PlayerRoleAlreadyExist())
        }

        val result = playerRoleRepository.addPlayerRole(context, PlayerRole(
               displayName = action.displayName,
               code = action.code
            )
        )

        return ResultProvider(
            status = if (result != null) ActionResultStatus.Success() else ActionResultStatus.FailedToCreatePlayerRole(),
            result = result
        )
    }

    override suspend fun removePlyaerRoleByCode(code: String): ResultProvider<Boolean> {
        /* Check role existence by code */
        playerRoleRepository.isExistByCode(code).also {
            if (it) return@also

            return ResultProvider(status = ActionResultStatus.PlayerRoleNotExist())
        }

        return ResultProvider( // TODO: Need to revise
            status = ActionResultStatus.Success(),
            result = playerRoleRepository.deleteByCode(code)
        )
    }

    override suspend fun updatePlayerRole(context: UserContext, action: UpdatePlayerRoleActionDTO): ResultProvider<PlayerRole> {
        val role = (playerRoleRepository.getSingleByCode(action.targetCode)
            ?: return ResultProvider(status = ActionResultStatus.PlayerRoleNotExist()))
            .apply {
                action.newDisplayName?.let {
                    this.displayName = it
                }

                action.newCode?.let {
                    this.code = it
                }
            }

        return ResultProvider(
            status = ActionResultStatus.Success(),
            result = playerRoleRepository.updatePlayerRole(context, role)
        )
    }

    override suspend fun getPlayerRoleByCode(code: String): ResultProvider<PlayerRole> {
        return ResultProvider(
            status = ActionResultStatus.Success(),
            result = playerRoleRepository.getSingleByCode(code)
        )
    }

    override suspend fun getAllPlayerRoles(): ResultProvider<ArrayList<PlayerRole>> {
        return ResultProvider(
            status = ActionResultStatus.Success(),
            result = playerRoleRepository.getAll()
        )
    }

}