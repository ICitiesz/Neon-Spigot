...
data class NWaypoints(private val waypointData: Map.Entry<String, JSONObject>) {
    ...
    object Handler {
        ...
        var canCrossDimension by Delegates.notNull<Boolean>()
                    ...
                    Handler.getNWaypointsData().forEach { waypointData ->
                        ...
                        if (!Handler.canCrossDimension) {
                            if (player.location.world!!.environment.toString().equals(modifiedLocation.world!!.environment.toString(), true)) {
                                Handler.teleportToWaypoint(
                                    player, modifiedLocation, goldWaypointName,
        ...
        override fun setGUIClickHandler(e: InventoryClickEvent) {
            ...
            when (currentItem.type) {
                ...
                Material.BLAZE_POWDER -> {
                    ...
                    neon.server.broadcastMessage(CommandSyntax.createSyntaxMessage("${ChatColor.RED}The " +
