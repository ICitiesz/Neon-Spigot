...
import com.islandstudio.neon.stable.core.init.SupportedVersions
...
object NItemGlinter {
    ...
    private val registriesBaseClass = NReflector.getNamespaceClass(NMS_REGISTRIES_BASE_CLASS_PATH)!!
    ...
    private val enchantmentRegistry = run {
        ...
        val registriesClass = when {
            NReflector.hasNamespaceClass(NMS_REGISTRIES_CLASS_PATH) -> {
                NReflector.getNamespaceClass(NMS_REGISTRIES_CLASS_PATH)
            }
        ...
        registriesClass.getField(NMSRemapped.Mapping.NMS_ENCHANTMENT_REGISTRY.remapped).get(null).also {
    ...
    object Handler {
        fun run() {
            ...
            run enableAddRegistryEntry@ {
                ...
                enchantmentRegistry.javaClass.getDeclaredField(NMSRemapped.Mapping.NMS_REGISTRY_ENTRY_TOGGLE_STATE.remapped).also {
            ...
            run disableAddRegistryEntry@ {
                ...
                enchantmentRegistry.javaClass.getMethod(NMSRemapped.Mapping.NMS_REVOKE_REGISTRY_ENTRY.remapped).invoke(enchantmentRegistry)
            }
        }
        ...
        fun <T: Enchantment> registerItemGlinter(neonKeyGeneral: NeonKeyGeneral, enchantment: T) {
                ...
                Holder.Reference::class.java.methods
                    ...
                    .filter { method ->
                        if (NReflector.hasNamespaceClass(NMS_REGISTERY_HOLDER_OWNER_PATH)) {
                            method.parameterTypes.contains(HolderOwner::class.java) && method.parameterTypes.contains(Any::class.java)
                        } else {
            ...
            val registeredItemGlinterType = Registry::class.java.getMethod(
                NMSRemapped.Mapping.NMS_REGISTER_ENCHANTMENT.remapped, net.minecraft.core.Registry::class.java, String::class.java, Any::class.java)
            ...
            val craftBukkitEnchant = NReflector.CraftBukkitReflector.getCraftBukkitClass("enchantments.CraftEnchantment")
                .getConstructor(net.minecraft.world.item.enchantment.Enchantment::class.java).newInstance(registeredItemGlinterType)
...
//    }
    ...
    fun applyItemGlinter(itemStack: ItemStack, itemGlinterType: ItemGlinterType): ItemMeta {
        ...
        val craftItemStack = NReflector.CraftBukkitReflector.getCraftItemStackClass()

        /* Apply enchantment (Item Glinter) */
        nmsItemStack.javaClass.getMethod(
            NMSRemapped.Mapping.NMS_APPLY_ENCHANTMENT.remapped,
    ...
    fun initOrGetUnregisterHolderContainer(enchantmentRegistry: Registry<Enchantment>, isInit: Boolean = true): IdentityHashMap<Any, Any>? {
        ...
        enchantmentRegistry.javaClass.declaredFields
            ...
            .filter { declaredField ->
                ...
                actualTypeArgs.first().typeName.equals(NMS_REGISTRIES_GENERIC_TYPE_T) && actualTypeArgs.last().typeName.contains(NMS_REGISTERY_HOLDER_CLASS_PATH)
            }.ifNotEmpty {
                /* Set the unregisterHolderContainer */
                this.first().apply {
                    this.isAccessible = true
